...
my @command = ( {'cwd' => "gcc-$gcc_version",
                 ...
                 'cmd' => "autoconf_version=`autoconf --version | head -n1 | grep -oE '[0-9]+\\.[0-9]+(\\.[0-9]+)?' | head -n1`; sed -i.bak -e \"s/AC_PREREQ(\\[2\\.69\\])/AC_PREREQ(\\[\$autoconf_version\\])/g\" -e \"s/AC_PREREQ(2\\.69)/AC_PREREQ(\$autoconf_version)/g\" -e \"s/AC_PREREQ(\\[2\\.64\\])/AC_PREREQ(\\[\$autoconf_version\\])/g\" -e \"s/AC_PREREQ(2\\.64)/AC_PREREQ(\$autoconf_version)/g\" -e \"s/m4_define(\\[_GCC_AUTOCONF_VERSION\\], \\[2\\.64\\])/m4_define([_GCC_AUTOCONF_VERSION], [\$autoconf_version])/g\" -e \"s/m4_define(\\[_GCC_AUTOCONF_VERSION\\], \\[2\\.69\\])/m4_define([_GCC_AUTOCONF_VERSION], [\$autoconf_version])/g\" -e \"s/Please use exactly Autoconf 2\\.64 instead of/Please use exactly Autoconf \$autoconf_version instead of/g\" -e \"s/Please use exactly Autoconf 2\\.69 instead of/Please use exactly Autoconf \$autoconf_version instead of/g\" configure.ac ./config/override.m4 2>/dev/null || true",
                 'arch' => 'all'},
                {'cwd' => "binutils-$binutils_version",
...
my @compile = ( {'dir' => "nasm-$nasm_version",
                ...
                {'dir' => "gcc-$gcc_version",
                 ...
                 'name' => "libstdc++",
                 'configure' => "--target=\$TARGET $gcc_configure_special --prefix=\$PREFIX --disable-nls --enable-languages=c++ --disable-libstdcxx-pch --disable-executables --enable-shared --enable-lto --with-sysroot",

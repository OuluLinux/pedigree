...
my @command = ( {'cwd' => "gcc-$gcc_version",
                 ...
                 'cmd' => "autoconf_version=`autoconf --version | head -n1 | grep -oE '[0-9]+\\.[0-9]+(\\.[0-9]+)?' | head -n1`; sed -i.bak -e \"s/AC_PREREQ(\\[2\\.64\\])/AC_PREREQ(\\[\$autoconf_version\\])/g\" -e \"s/AC_PREREQ(2\\.64)/AC_PREREQ(\$autoconf_version)/g\" -e \"s/Please use exactly Autoconf 2\\.64 instead of/Please use exactly Autoconf \$autoconf_version instead of/g\" configure.ac ./config/override.m4 2>/dev/null || true",
                 'arch' => 'all'},
                 {'cwd' => "binutils-$binutils_version",
                 'name' => "fixing autoconf version dependency (Binutils)",
                 'cmd' => "autoconf_version=`autoconf --version | head -n1 | grep -oE '[0-9]+\\.[0-9]+(\\.[0-9]+)?' | head -n1`; sed -i.bak -e \"s/AC_PREREQ(\\[2\\.69\\])/AC_PREREQ(\\[\$autoconf_version\\])/g\" -e \"s/AC_PREREQ(2\\.69)/AC_PREREQ(\$autoconf_version)/g\" -e \"s/AC_PREREQ(\\[2\\.64\\])/AC_PREREQ(\\[\$autoconf_version\\])/g\" -e \"s/AC_PREREQ(2\\.64)/AC_PREREQ(\$autoconf_version)/g\" configure.ac ./config/override.m4 2>/dev/null || true",
                 'arch' => 'all'},
                {'cwd' => "binutils-$binutils_version",

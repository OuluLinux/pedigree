...
my @command = ( {'cwd' => "gcc-$gcc_version",
                 ...
                 'cmd' => "autoconf_version=`autoconf --version | head -n1 | grep -oE '[0-9]+\\.[0-9]+(\\.[0-9]+)?' | head -n1`; export autoconf_version; sed -i.bak \"s/m4_version_prereq(\\[2\\.69\\])/m4_version_prereq(\\[\\\$autoconf_version\\])/g; s/m4_version_prereq(2\\.69)/m4_version_prereq(\\\$autoconf_version)/g; s/AC_PREREQ(\\[2\\.69\\])/AC_PREREQ(\\[\\\$autoconf_version\\])/g; s/AC_PREREQ(2\\.69)/AC_PREREQ(\\\$autoconf_version)/g; s/Please use exactly Autoconf 2\\.69 instead of/Please use exactly Autoconf \\\$autoconf_version instead of/g\" configure.ac ./config/override.m4 2>/dev/null || true",
                 'arch' => 'all'},
                {'cwd' => "binutils-$binutils_version",
